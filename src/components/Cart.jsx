...
const Cart = () => {
    ...
    return (
        <div
            className={`fixed top-0 right-0 z-50 flex justify-end w-screen h-screen overflow-auto no-scrollbar transition-opacity duration-300 ease-in-out  ${
        ...
        >
            ...
            <div
                className={`h-full min-w-[400px] max-w-[500px] bg-white transition-transform duration-500 ease-in-out flex flex-col ${
                    cartIsOpen ? 'translate-x-0' : 'translate-x-full'
                }`}
            >
                <div className="flex justify-between items-center z-50 text-base uppercase font-medium p-8 py-4 border-b-[1px] border-gray-300 sticky top-0 bg-white">
                ...
                {cartItems.length === 0 ? (
                    <div className="flex-grow flex items-center justify-center">
                        <div className="text-center">
                            ...
                            <button onClick={() => setCartIsOpen(false)} className="bg-black text-white px-6 py-2">
                                Continue Shopping
                            </button>
                        </div>
                    </div>
                                ...
                                {cartItems.map((item, key) => (
                                    <div key={key} className="flex w-full gap-4">
                                        ...
                                        <div className="font-medium text-[15px] w-full flex flex-col gap-1">
                                            <div>{formatToIndianCurrency(item.price * item.itemCount)}</div>
